...
class BasicCache(database: Database) {
  ...
  private[this] val cache: AsyncLoadingCache[ListingKey, Json] = Scaffeine()
    ...
    .buildAsyncFuture[ListingKey, Json](loader = {
      case UpcomingMatches ⇒ database.run(listUpcomingMatchesQuery).map(_.asJson)(database.ec)
      case CurrentUbl      ⇒ database.run(database.getCurrentUbl).map(_.asJson)(database.ec)
